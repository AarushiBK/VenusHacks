...
export function detectEmergency(userMessage: string): EmergencyResult {
  ...
  const isEmergency = maxSeverity !== "none";

  return {
    isEmergency,
    severity: maxSeverity,
    matchedContexts,
    action: isEmergency ? toNavigateAction("EmergencyResources") : undefined,
  };
}
...
export function buildEmergencyResponse(result: EmergencyResult): AssistantResponse {
  ...
  return {
    ...
    type: "emergency",
    citations: [],
    actions: result.action ? [result.action] : [toNavigateAction("EmergencyResources")],
    voiceTone: "urgent",
  };
}
