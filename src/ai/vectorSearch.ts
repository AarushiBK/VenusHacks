...
function loadBundledIndex(): DocumentChunk[] {
  const indexPath = join(process.cwd(), "data", "vector-index.json");
  try {
    if (existsSync(indexPath)) {
      const raw = readFileSync(indexPath, "utf-8");
      const bundled = JSON.parse(raw) as DocumentChunk[];
    ...
    console.warn("[Hera] Could not load data/vector-index.json — using default knowledge");
  }
