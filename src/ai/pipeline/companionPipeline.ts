...
export async function runCompanionPipeline(
  ...
  if (
    ...
    console.log("[Vena Companion] Gemini unavailable or incomplete  offline fallback");
...
function inferActions(
  ...
  if (mode === "APP_ACTION") {
    if (/\b(symptom|mood)\b/i.test(userMessage)) {
      return [{ type: "navigate" as const, screen: "SymptomTracker" as const }];
    }
    if (/\b(bp|blood pressure|heart|bpm|wearable)\b/i.test(userMessage)) {
      return [{ type: "navigate" as const, screen: "HeartDashboard" as const }];
    }
  }
  ...
  if (
    ...
    return [{ type: "navigate" as const, screen: "EmergencyResources" as const }];
  }
